...
# 1. 定义屏蔽词列表（可以自定义或扩展）
stopwords1 = ['the', 'and', 'of', 'in', 'to', 'for', 'a', 'on', 'is', 'with', 'at', 'this', 'an', 's', 'es', 'neo', 'index',
             'urban', 'renewal', 'simulation', 'city', 'project', 'base', 'model', 'study', 'analysis', 'cities', 'services',
             'development', 'redevelopment', 'process', 'public', 'result', 'provide', 'area', 'research', 'policies', 'into',
             'method', 'building', 'change', 'planning', 'datum', 'data', 'design', 'high', 'low', 'paper', 'making', 'local',
             ...
             'politics', 'participate', 'participation', 'rights', 'state', 'gen', 'post', 'projects', 'land', 'cover', 'two',
             ...
             'driving factors', 'not only', 'time series', 'patch generating', 'through', 'such', 'been', 'have been', 'built',
             'difference', 'differences', 'or', 'existing', 'structure', 'buildings', 'potential', 'factors', 'studies',
             'time', 'street', 'show', 'support', 'findings', 'strategies', 'government', 'within']
stopwords2 = ['view images', 'structural racism', 'random forest', 'carried out', 'hong kong', 'new york', 'coordination degree',
              'overall accuracy', 'recent years', 'difference differences']
stopwords3 = nltk.download('stopwords')
nltk_stopwords3 = set(stopwords.words('english'))
# 2. 加载 BERT(-T5L) 分词器
tokenizer = BertTokenizer.from_pretrained('bert-large-uncased')
# tokenizer = T5Tokenizer.from_pretrained('t5-large')
stopwords_subwords = nltk_stopwords3.union(set(stopwords1+stopwords2))
...
def main_proceed():
    ...
    # 6. 指定需要处理的文本列（支持多列）
    columns_to_process = [f'keyword{i}' for i in range(18)] + ['abstract']  # 替换为你的列名列表
    # 7. 处理多列文本数据
...
if __name__=='__main__':
    ...
    # 12. 输出前20个最常见的词
    print("前20个最常见的词频：", dict(list(word_freq_list.items())[:20]))
    # 13. 绘制词频图
    rcParams['font.sans-serif'] = ['SimHei', 'Times New Roman']  # 可以根据需要换成其他字体
    rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
    plt.figure(figsize=(16, 12))
    # 提取前20个高频二元短语及其频率
    phrases = list(word_freq_list.keys())[:20]
    frequencies = list(word_freq_list.values())[:20]
    # 绘制横向条形图
    ...
    pd.DataFrame(list(word_freq_list.items()),columns=['Phrase', 'Frequency']).to_csv("tables/urban_renewal_frequency_OFFICIAL.csv",
                                                                                      index=False)
    plt.show()
